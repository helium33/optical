...
import PageLoader from "../../../Component/PageLoader"; // Add missing import for consistency
...
const Eyeglasses = () => {
  ...
  const { data: products, error: productError } = useSWR("http://localhost:3001/eyeglasses", fetcher);
  ...
  if (!navItems || !products) return <PageLoader />; // Use PageLoader for consistency
  ...
  const inferCategory = (product, category) => {
    const text = `${product.name} ${product.description}`.toLowerCase();
    switch (category.toLowerCase()) {
      case "men":
        return text.includes("business") || text.includes("professional") || text.includes("sport");
      case "women":
        return text.includes("cat eye") || text.includes("butterfly") || text.includes("glam");
      case "sunglasses":
        return text.includes("sport shield") || text.includes("aviator") || text.includes("polarized");
      case "uv protection":
        return text.includes("polarized") || text.includes("shield");
      case "best seller":
        return product.stock > 40;
      case "new arrivals":
        return text.includes("modern") || text.includes("trend");
      case "blue light blocking":
        return text.includes("anti-glare") || text.includes("tech");
      default:
        return true;
    }
  };
  ...
  const filteredProducts = products
    .filter((product) => selectedCategory === "all" || inferCategory(product, selectedCategory))
    .filter((product) =>
        ...
        {filteredProducts.map((product) => (
          <div key={product.id} className="border p-4 rounded shadow-md">
            ...
            <div className="mt-3 flex items-center justify-between gap-2">
              <Link to={`/eyeglasses/${product.id}`} className="text-blue-500 hover:text-blue-700 font-semibold">
                View Details
              </Link>
              <button
